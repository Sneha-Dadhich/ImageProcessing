...
# making the dataset
face = cv2.CascadeClassifier(r"DRDO\codes\face_detection_model\haarcascade_frontalface_default.xml")
...
while(True):
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.imwrite(fr"DRDO\codes\face_detection_model\dataset\user.{str(id)}.{str(id)}.{str(picture)}.{str(name)}.jpg", gray[y:y+h, x:x+w]) 
